<template>
  <div class="d-container news-container" flex="dir:top box:frist">
    <van-nav-bar
      :title="titlename + '资讯'"
      left-arrow
...
export default {
  ...
  created() {
    ...
    const params = JSON.parse(sessionStorage.getItem('news-params'))
    if (params) {
  ...
  methods: {
    ...
    toNews(item) {
      ...
      sessionStorage.setItem('news-params', JSON.stringify(params))
      this.$router.push({
        name: 'NewsDetail',
