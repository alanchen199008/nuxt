<template>
  <div flex="dir:top">
    <van-nav-bar title="客队进球声音" left-text="" left-arrow @click-left="onClickLeft" />
...
export default {
  ...
  methods: {
    ...
    handleUpdate() {
      if (this.memberId) {
        updateMatchRingtoneWithFootball({ guestGoalSound: this.radio })
          .then(([_, error]) => {
            if (!error) {
              this.$toast('修改客队提示音成功！')
              setTimeout(() => {
                this.$router.push({ name: 'MatchRingtone' })
              }, 1000)
            }
      ...
      this.prompt.guestGoalSound = this.radio
      this.$toast('修改客队提示音成功！')
      setTimeout(() => {
        this.$router.push({ name: 'MatchRingtone' })
      }, 1000)
