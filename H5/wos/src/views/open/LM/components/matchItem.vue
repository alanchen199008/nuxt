<template>
  <div class="match-item van-hairline--bottom van-hairline--top-bottom" @click="toDetail(item.matchId)">
    <div class="match-item-item" flex="cross:top box:mean">
      ...
      <div flex-box="1" class="match-item__center" flex="cross:center box:mean">
        ...
        <div flex-box="0" class="match-item__center__middle" flex="main:center">
          ...
          <span v-if="item.status === 1" class="color-red-regular">未赛</span>
      ...
      <div flex-box="0" class="match-item__right">
        ...
        <div class="tgbox" v-if="item.hasIntelligence === 1 || item.recommendCount>0">
          <span v-if="item.hasIntelligence === 1" class="match-tag">情报</span>
          <span v-if="recommendCount > 0" class="match-tag">
            推荐 {{ recommendCount &lt; 100 ? recommendCount : '99+' }}
          </span>
        </div>
    ...
    <div class="match-item-item" flex="cross:top box:mean">
      <div flex-box="0" class="match-item__left">
        <!-- <img class="wen" src="~@/assets/LM/wen.png"> -->
        <div class="match-item__center__left"  style="margin-left:auto">
          ...
          <span flex-box="1" class="team-name">{{ item.homeTeamName }}</span>
        </div>
      </div>
      <div flex-box="1" class="match-item__center" flex="cross:top box:mean">
        ...
        <div flex-box="0" class="match-item__center__middle" flex="main:center">
          <span class="match-center">
            ...
            <span v-else class="match-score">VS</span>
          </span>
        </div>
      ...
      <div flex-box="0" class="match-item__right">
        ...
        <img v-if="item.liveUrl && item.status !== 8" class="live-icon" src="@/assets/public/player-w.png" alt="">
      </div>
    </div>
    <div class="match-item-item" flex="cross:top box:mean">
      <div flex-box="0" class="match-item__left" />
...
export default {
  ...
  computed: {
    ...
    recommendCount() {
      const item = this.recommends.find((item, index) => {
          return item.matchId === this.item.matchId
      })
