...
export default {
  ...
  methods: {
    toDetail(id) {
      if (this.mode === 0) {
        this.$router.push({ name: 'HomeRecommend', params: { id }})
      }
      if (this.mode === 1) {
        window.android.intoRecommendDetail(id)
      }
      if (this.mode === 2) {
        window.webkit.messageHandlers.ShowExpertRecomandDetail.postMessage({ articleId: id })
      }
    }
  }
}
