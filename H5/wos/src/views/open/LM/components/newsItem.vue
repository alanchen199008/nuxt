<template>
  <div class="news-item" flex="cross:center box:mean" @click="handleClick(item.objectId)">
    <div class="news-item-content">
      ...
      <div class="sub-title" flex="main:justify" style="color:#959595FF">
        <!-- <span>{{ item.createTime | parseTime("{m}-{d} {h}:{i}") }}</span> -->
        <div>{{ item.newsShortTitle }} | {{ item.createTime | parseTime("{y}-{m}-{d}") }} | 阅读量{{ item.views >= 10000 ? "9999+" : item.views }}</div>
        <!-- <span class="views">
          <van-icon name="eye" />
          阅读量
